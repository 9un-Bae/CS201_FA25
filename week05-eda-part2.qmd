...
```{r}
ggplot(pokedata, aes(x = type_2, fill = generation)) +
  geom_bar() +           # bars + outlines
  theme_bw() +
  facet_wrap(~ type_1) +
  labs(title = "Pokémon by Type2 & Facet by Type1",
       x = "Type 2",
       y = "Count") +
...
What is the relationship here? - **More clarity but lower in price, maybe because more clarity but smaller in carat**

For `color` and `clarity`, is there more visible variation or covariation? - **Covariation because distribution of price changes with different categories**

Between `color`, `clarity`, and `carat`, which do you think is the best predictor of `price`? - **Carat**\
(If you took DS-303 last semester, you might even try a quick linear regression to check!)

```{r}
# Carat bins for boxplot
carat_bin <- diamonds %>%
  mutate(carat_bin = cut(carat, breaks = c(0,1,2,3,4,5), include.lowest = TRUE))
...
# Price vs C-binarat (binned)
ggplot(carat_bin, aes(x = carat_bin, y = price)) +
  geom_boxplot() +
  theme_bw() +
...
-   Where do we see a lot of variability? - **Fair and Good, the spread of carat values is much larger**

-   Is there a relationship between carat and cut? - **Higher cut -\> smaller carat, Lower cut -\> larger carat**

-   Which category has the largest carat diamonds? - **Fair because whisker and outliers go higher**
...
Answer: **Boxplot because I could easily compare how the distribution of price changed across categories like color and clarity**
