...
```{r, messages=FALSE}
filter <- penguins_data %>%
  filter(sex == "FEMALE", culmen_length_mm >= 39, island %in% c("Dream", "Torgersen"))

# Check results
unique(filter$sex)
unique(filter$island)
summary(filter$culmen_length_mm)
...
```{r, message=FALSE, warning=FALSE}
case_when <- penguins %>%
  mutate(size = dplyr::case_when(body_mass_g >= 4050 ~ "big", 
...
final_boss <- penguins %>%
  ...
  # make sure flipper_length_mm is not missing
  filter(sex == "FEMALE",
         !is.na(flipper_length_mm),
         # keep only penguins from Dream or Biscoe
         island %in% c("Dream", "Biscoe")) %>%
  
  select(species, island, flipper_length_mm, body_mass_g) %>%
  # create new column flipper_rank using case_when
  mutate(flipper_rank = case_when(flipper_length_mm < 190 ~ "short", 
                                  flipper_length_mm >= 190 & flipper_length_mm <= 210 ~ "medium", # medium if between 190–210
                                  flipper_length_mm > 210 ~ "long")) %>%
  
  group_by(species, island, flipper_rank) %>%
  # calculate average body mass and round the numbers
  summarise(avg_mass_g = round(mean(body_mass_g, na.rm = TRUE), 0))
