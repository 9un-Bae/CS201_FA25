...
ggplot(data = penguins_data, aes(x = flipper_length_mm, fill = species)) +
  geom_histogram(alpha = 0.7) + #alpha adds transparency
  theme_bw() +
...
ggplot(data = penguins_data, aes(x = flipper_length_mm, fill = species)) +
  geom_histogram() +
  theme_bw() + 
  scale_fill_viridis_d(option="E") + # try adding option = "A", "C", "E", etc. These also have names! See documentation.
  labs(title = "Palmer Penguins, Distribution of Flipper Length",
       subtitle = "3 penguin species observed on three islands in the Palmer Archipelago, Antarctica",
       x = "Flipper Length (mm)",
       y = "Number of Penguins")
...
```{r, message=FALSE, warning=FALSE}
 ggplot(penguins_data, aes(x = culmen_depth_mm, fill = species)) +
  geom_histogram(binwidth = 0.5, color = "black") +   # binwidth adjustable
...
```{r, message=FALSE, warning=FALSE}
ggplot(penguins_data, aes(x = culmen_depth_mm, y = body_mass_g, color = species)) +
  geom_point(size = 3) +
  scale_color_viridis_d(option = "cividis") +
  theme_bw() +
  labs(
    title = "Penguin Body Mass vs. Culmen Depth",
...
# build the ggplot
penguin_scatter <- ggplot(penguins_data, 
                          aes(x = culmen_depth_mm, y = body_mass_g, color = species)) +
