...
```{r, message=FALSE}
mass_by_flipper_facet <- ggplot(
  penguins_male_female,
  aes(
    x = flipper_length_mm,
    y = body_mass_g,
    color = species,
  ...
  labs(
    title = "Flipper Length vs Body Mass by Island and Sex",
    subtitle = "Color = Species, Shape = Island",
    x = "Flipper Length (mm)",
    y = "Body Mass (g)"
  )

ggplotly(mass_by_flipper_facet, tooltip = "text")
```
...
2.  Now, create a bar graph using the *new* dataframe you made
    ...
    ```{r}
    ggplot(data = average_mass, aes(x = species, y = average_body_mass, fill = species)) +
      # fill in the blanks
      ...
      theme_bw() +
      labs(
        title = "Average Penguin Body Mass by Species",
        x = "Species",
...
ggplot(data = average_flipper_length, aes(x = species, y = average_flipper_length, fill = species)) +
  ...
  labs(
    title = "Average Penguin Flipper Length by Species",
    x = "Species",
...
view(average_culmen_depth)
```

```{r}
ex4 <- ggplot(data = average_culmen_depth, aes(x = species, y = average_culmen_depth, fill = species)) +
  geom_bar(stat = "identity") +
  theme_bw() +
  labs(
    title = "Average Culmen Depth",
    x = "Species",
